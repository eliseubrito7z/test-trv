"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GoogleOauthModule = void 0;
const common_1 = require("@nestjs/common");
const auth_google_controller_1 = require("./auth-google.controller");
const auth_google_repository_1 = require("./auth-google.repository");
const auth_google_strategy_1 = require("./auth-google.strategy");
const prisma_service_1 = require("../../infra/database/prisma.service");
const googleLogin_usecase_1 = require("./useCases/googleLogin.usecase");
let GoogleOauthModule = class GoogleOauthModule {
};
exports.GoogleOauthModule = GoogleOauthModule;
exports.GoogleOauthModule = GoogleOauthModule = __decorate([
    (0, common_1.Module)({
        imports: [],
        controllers: [auth_google_controller_1.GoogleController],
        providers: [auth_google_repository_1.GoogleRepository, auth_google_strategy_1.GoogleStrategy, prisma_service_1.PrismaService, googleLogin_usecase_1.GoogleLoginUseCase],
    })
], GoogleOauthModule);
//# sourceMappingURL=auth-google.module.js.map